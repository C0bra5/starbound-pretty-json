...
formatter.parseAndValidate = (text) ->
  ...
  try
    ...
    if atom.config.get 'starbound-pretty-json.notifyOnParseError'
...
PrettyJSON =
  ...
  doEntireFile: (editor) ->
    grammars = atom.config.get 'starbound-pretty-json.grammars' ? []
  ...
  activate: ->
    atom.commands.add 'atom-workspace',
      'starbound-pretty-json:prettify': =>
        editor = atom.workspace.getActiveTextEditor()
        @prettify editor,
          entire: @doEntireFile editor
          sorted: false
          selected: true
      'starbound-pretty-json:minify': =>
        editor = atom.workspace.getActiveTextEditor()
        @minify editor,
          entire: @doEntireFile editor
          selected: true
      'starbound-pretty-json:sort-and-prettify': =>
        editor = atom.workspace.getActiveTextEditor()
        @prettify editor,
          entire: @doEntireFile editor
          sorted: true
          selected: true
      'starbound-pretty-json:jsonify-literal-and-prettify': =>
        editor = atom.workspace.getActiveTextEditor()
        @jsonify editor,
          entire: @doEntireFile editor
          sorted: false
          selected: true
      'starbound-pretty-json:jsonify-literal-and-sort-and-prettify': =>
        editor = atom.workspace.getActiveTextEditor()
        @jsonify editor,
          entire: @doEntireFile editor
          sorted: true
    ...
    @subscriptions.add atom.config.observe 'starbound-pretty-json.prettifyOnSaveJSON', (value) =>
